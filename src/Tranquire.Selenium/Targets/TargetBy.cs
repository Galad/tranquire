...
public class TargetBy : ITarget
{
    ...
    /// </summary>
    /// <returns></returns>
    public override string ToString() => Name + $" ({By})";
    ...
    public ITarget RelativeTo(ITarget targetSource)
    {
        ...
        static string AsCssSelector(string value, ByType byType)
        {
            switch (byType)
            {
                case ByType.Id:
    ...
    private static readonly Regex _byDescriptionRegex = new(@"(?<type>By\.[^:]*):\s(?<value>.*)", RegexOptions.Compiled);
    private static readonly Dictionary<string, ByType> _byTypeMap = new()
    {
        { "By.Id", ByType.Id },
