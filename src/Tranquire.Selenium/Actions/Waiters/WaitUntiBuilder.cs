...
public sealed class WaitUntilTargetBuilder
{
    ...
    /// </summary>
    public WaitUntilQuestionIsAnswered<bool> IsVisible => new(Visibility.Of(_target), isVisible => isVisible);

    /// <summary>
    /// Wait until the target is not visible
    /// </summary>
    public WaitUntilQuestionIsAnswered<bool> IsNotVisible => new(Visibility.Of(_target), isVisible => !isVisible);

    /// <summary>
    /// Wait until the given target is present in the DOM
    /// </summary>
    public WaitUntilQuestionIsAnswered<bool> IsPresent => new(Presence.Of(_target), isPresent => isPresent);
}