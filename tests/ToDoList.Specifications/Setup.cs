...
public class Setup : StepsBase
{
    ...
    public static void StartWebServer()
    {
        ...
        // Remove the "\\bin\\Debug\\<framework>"
        ...
        var currentDirectory = Path.Join(solutionPath, @"src\ToDoList");
        Directory.SetCurrentDirectory(currentDirectory);

        var webHostBuilder = WebHost.CreateDefaultBuilder()
                                    .UseUrls(Open.RootUrl)
                                    .UseWebRoot(Path.Combine(currentDirectory, "ClientApp"))
                                    .UseStartup<Startup>()
        ...
        if (TestLevel == TestLevel.UI)
        {
            ...
            ClientFactory = () => new HttpClient() { BaseAddress = new Uri(Open.RootUrl, UriKind.Absolute) };
        }
